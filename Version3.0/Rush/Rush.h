...
bool JJStock::removeFromStock(JJPieces check){
	for (std::list<JJStockPiece>::iterator itr = units.begin(); itr != units.end(); ++itr)
	{
...
public:
	...
	std::list<JJAction> actionList;	// Full set of actions
	std::list<JJAction> speedQueue;	// Actions currently being executed/to be executed next
	std::list<JJAction> placements;  // List of placements actions for the end of the next turn.
	std::list<JJAction> knightMoves; // List of knight moves. Needed because they dodge conflict except on the landing square.
...
void JJRush::placeKnights(){
	for (std::list<JJAction>::const_iterator itr = placements.begin(), end = placements.end(); itr != end; ++itr) {
		JJAction temp = *itr;
...
void JJRush::ResolveAll(){
	for (int i = 0; i > 12; i++){
		...
		for (std::list<JJAction>::const_iterator itr = actionList.begin(), end = actionList.end(); itr != end; ++itr) {
			if (!gameOver){
				JJAction temp = *itr;
...
void JJRush::ResolveSpeed(int speed){
	...
	case 0:
		for (std::list<JJAction>::const_iterator i = speedQueue.begin(), end = speedQueue.end(); i != end; ++i) {
			JJAction temp = *i;
