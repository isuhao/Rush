...
bool JJStock::removeFromStock(JJPieces check){
	int loc = units.FindElement(&JJStockPiece(check));
	if (loc != -1){
		units.RemoveElement(loc);
		return true;
	}
	return false;
}
...
void JJStock::addToStock(int respawn, JJPieces type){
	units.AddElement(JJStockPiece(type, respawn));
};

JJStock::JJStock(){
	units.AddElement(JJStockPiece(pawn));
	units.AddElement(JJStockPiece(pawn));
	units.AddElement(JJStockPiece(pawn));
	units.AddElement(JJStockPiece(pawn));
	units.AddElement(JJStockPiece(pawn));
	units.AddElement(JJStockPiece(knight));
	units.AddElement(JJStockPiece(knight));
	units.AddElement(JJStockPiece(rook));
	units.AddElement(JJStockPiece(rook));
	units.AddElement(JJStockPiece(bish));
	units.AddElement(JJStockPiece(bish));
	units.AddElement(JJStockPiece(king));
	units.AddElement(JJStockPiece(queen));
}
...
JJStock::JJStock(JJArmies army){
	...
	case battleship:
		...
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(torp));
		units.AddElement(JJStockPiece(dest));
		units.AddElement(JJStockPiece(salv));
		units.AddElement(JJStockPiece(salv));
		units.AddElement(JJStockPiece(battle));
		units.AddElement(JJStockPiece(aircc));
		units.AddElement(JJStockPiece(aircc));
		break;
	}
}
...
public:
	...
	C4::Array<JJAction *> actionList;	// Full set of actions
	C4::Array<JJAction *> speedQueue;	// Actions currently being executed/to be executed next
	C4::Array<JJAction *> placements;  // List of placements actions for the end of the next turn.
	C4::Array<JJAction *> knightMoves; // List of knight moves. Needed because they dodge conflict except on the landing square.
...
void JJRush::placeKnights(){
	...
	for (int i = 0; i < knightMoves.GetElementCount(); i++) {
		JJAction *temp = static_cast<JJAction *>(&(*knightMoves)[i]);
...
void JJRush::submitAction(JJaTypes aType,JJTeams team, JJPieces type, int x1, int y1, int x2, int y2){
	...
	if (aType == movement)
		actionList.AddElement(temp);
	else
		placements.AddElement(temp);
};
...
void JJRush::ResolveAll(){
	for (int i = 0; i > 12; i++){
		...
		for (int i = 0; i < actionList.GetElementCount(); i++) {
			if (!gameOver){
				JJAction *temp = static_cast<JJAction *>(&(*actionList)[i]);
				if (isThere(temp->type, temp->colour, temp->x1, temp->y1)){			//Check to make sure pieces is still alive before submitting action.
					...
					case 0:
						...
						if (temp->type == pawn) {
							if (board[temp->x2][temp->y2].type != empty){
								speedQueue.AddElement(temp);
								int loc = actionList.FindElement(temp);
								actionList.RemoveElement(loc);
							}
		...
		for (int i = 0; i < placements.GetElementCount(); i++)
		{
			JJAction *temp = static_cast<JJAction *>(&(*placements)[i]);
			if (board[temp->x2][temp->y2].type == empty){
...
void JJRush::ResolveSpeed(int speed){
	...
	case 0:
		for (int i = 0; i < speedQueue.GetElementCount(); i++)
		{
			JJAction *temp = static_cast<JJAction *>(&(*speedQueue)[i]);
...
bool JJmoveCheck(JJRush gameBoard, JJPieces type, JJTeams team, int x1, int y1, int x2, int y2){
	int yDiff = y1 - y2;	// These two statements repeated so much in my initial if statements
	if (yDiff < 0){ yDiff = -yDiff; }
	int xDiff = x1 - x2;	// I figured I might as well abstract them.
	...
	*/
	...
	case salv:
		...
		return false;
	case battle:
		yDiff = y1 + forward - y2;								//Fixes check for automove
		if (yDiff < 0){ yDiff = -yDiff; }
