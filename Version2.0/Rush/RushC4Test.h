...
void JJStock::addToStock(int respawn, JJPieces type){
	units.Append(JJStockPiece(type, respawn));
};

JJStock::JJStock(){
	units.Append(JJStockPiece(pawn));
	units.Append(JJStockPiece(pawn));
	units.Append(JJStockPiece(pawn));
	units.Append(JJStockPiece(pawn));
	units.Append(JJStockPiece(pawn));
	units.Append(JJStockPiece(knight));
	units.Append(JJStockPiece(knight));
	units.Append(JJStockPiece(rook));
	units.Append(JJStockPiece(rook));
	units.Append(JJStockPiece(bish));
	units.Append(JJStockPiece(bish));
	units.Append(JJStockPiece(king));
	units.Append(JJStockPiece(queen));
}
...
JJStock::JJStock(JJArmies army){
	...
	case battleship:
		...
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(torp));
		units.Append(JJStockPiece(dest));
		units.Append(JJStockPiece(salv));
		units.Append(JJStockPiece(salv));
		units.Append(JJStockPiece(battle));
		units.Append(JJStockPiece(aircc));
		units.Append(JJStockPiece(aircc));
		break;
	}
}
			...
			if (!gameOver){
				...
				if (isThere(temp.type, temp.colour, temp.x1, temp.y1)){			//Check to make sure pieces is still alive before submitting action.
					...
					case 0:
						...
						if (temp.type == pawn) {
							if (board[temp.x2][temp.y2].type != empty){
								speedQueue.Append(temp);
								itr = actionList.Remove(itr);
							}
